...
export const getAllEntities = createAsyncThunk('products/fetch_all_entity_list', async (id?: string) => {
  if (id) return api.get<IOption[]>(`${apiUrl}/all?id=${id}`);
  return api.get<IOption[]>(`${apiUrl}/all`);
});
